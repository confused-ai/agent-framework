...
describe('tenantScopedKey', () => {
  ...
  it('throws when tenantId contains colon separator', () => {
    expect(() => tenantScopedKey('t1:evil', 'session', 'abc')).toThrow(/must not contain/);
  });

  it('throws when a part contains colon separator', () => {
    expect(() => tenantScopedKey('t1', 'sess:ion', 'abc')).toThrow(/must not contain/);
  });
});
