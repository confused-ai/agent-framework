...
describe('RedisRateLimiter unit behavior', () => {
    class FakeEvalClient {
        ...
        async eval(
            ...
            const windowStart = Number(options.arguments[1]!);
            const maxRequests = Number(options.arguments[3]!);
            const member = options.arguments[4] ?? options.arguments[0]!;
            const existing = this.sets.get(key) ?? new Map<string, number>();

            for (const [entryMember, score] of existing) {
                if (score <= windowStart) existing.delete(entryMember);
            }

            if (existing.size < maxRequests) {
                existing.set(member, now);
    ...
    it('counts same-millisecond requests independently', async () => {
