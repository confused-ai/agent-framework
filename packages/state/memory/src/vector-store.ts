...
export class VectorMemoryStore implements MemoryStore {
    ...
    async get(id: EntityId): Promise<MemoryEntry | null> {
        ...
        // Ask the adapter directly (works for InMemoryVectorStore and any adapter
        // that implements the optional get() method)
        if (this.vectorStore.get) {
            const vec = await this.vectorStore.get(id);
            if (vec) {
                const entry = this.reconstructEntry(id, vec.metadata);
