    ...
    switch (typeName) {
        case 'ZodString':
            return {
                ...
                ...(def.checks?.some((c) => c.kind === 'min') && {
                    minLength: def.checks.find((c) => c.kind === 'min')?.value,
                }),
                ...(def.checks?.some((c) => c.kind === 'max') && {
                    maxLength: def.checks.find((c) => c.kind === 'max')?.value,
                }),
                ...(def.checks?.some((c) => c.kind === 'email') && { format: 'email' }),
                ...(def.checks?.some((c) => c.kind === 'url') && { format: 'uri' }),
                ...(def.checks?.some((c) => c.kind === 'regex') && {
                    pattern: def.checks.find((c) => c.kind === 'regex')?.regex?.source,
                }),
            };
        ...
        case 'ZodNumber':
            return {
                ...
                ...(def.checks?.some((c) => c.kind === 'int') && { type: 'integer' }),
            };
